...
def _decrement(quantity: float, deps: LightspeedDeps) -> Callable[[Entity], None]:
    ...
    def mutate(draft: Entity) -> None:
        current = draft.get(_INVENTORY_LEVEL)
        level = float(current) if isinstance(current, int | float) and not isinstance(current, bool) else 0.0
        moved = level + quantity
        # An integer level stays an integer on the wire: the schema types the
        # member `number`, and a shop that counts units should not start
        # reading 6.0 where it read 7.
        draft[_INVENTORY_LEVEL] = int(moved) if moved == int(moved) else moved
        stamp_version(draft, deps)
...
def _example_body() -> dict[str, Any]:
    ...
    return {
        ...
        "line_items": [
            {
                "product": {"id": SEED_PRODUCT_COFFEE_ID},
                "quantity": 1,
                "pricing": {"price": 4.5},
                "tax": {"id": SEED_TAX_ID, "amount": 0.68},
            }
        ],
    }
