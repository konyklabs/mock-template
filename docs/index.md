...
The word *fake* is precise rather than modest: a **fake** has a working
implementation — real state, real transitions, real consequences. Create an
order and it exists; pay it and the webhook fires, signed the way the vendor
signs it. Where the public documentation is silent and a behaviour had to be
decided, the wire says so — see
...
- **[Start here](start/install.md)** — install it, make the first request,
  ...
  container) that fits your test suite, with the pytest plugin, docker
  compose, CI and TLS on the same page.
- **[Concepts](concepts/unit.md)** — the vocabulary in four pages:
  [unit](concepts/unit.md) (profile, capabilities and roles, driver, journal
  and request log, clock), [seed](concepts/seed.md) (overlays and the seeded
  scenario per vendor), [chaos](concepts/chaos.md) (rules, faults, in-band
  triggers, provenance labels), and [fidelity](concepts/fidelity.md).
- **[Testing](testing.md)** — the tiers this repository's own suite runs.
...
behaviour a generic stand-in doesn't have.

The architecture is recorded as ADRs in the
[roadmap](https://github.com/konyklabs/roadmap/tree/main/decisions): **D-001**
(stateful vendor units with a shared core, a journal-backed state engine,
capability profiles, deterministic chaos) and **D-002** (Python on FastAPI, a
single distribution with vendors as modules). The invariant they turn on: the
stateful machinery stays framework-free and the web framework lives only in
the transport adapter, enforced in CI with import-linter and an AST-level
boundary check.

Not on PyPI or a container registry yet. Treat interfaces as subject to change
before v1. Apache-2.0 — see `LICENSE`.
