...
def test_every_constant_names_something_the_document_contains(document: dict[str, Any]) -> None:
    ...
    assert {account.id for account in doc.loyalty_accounts} == {
        seed_constants.SEED_LOYALTY_ACCOUNT_ID,
        seed_constants.SEED_LOYALTY_SECOND_ACCOUNT_ID,
    }
    account = next(a for a in doc.loyalty_accounts if a.id == seed_constants.SEED_LOYALTY_ACCOUNT_ID)
