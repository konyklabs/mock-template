...
def line(
    product_id: str = c.SEED_PRODUCT_TRAIL_MIX_ID, quantity: float = 1, price: float = 4.5, tax: float = 0.68
) -> Any:
    return {
...
def test_a_sale_carries_the_documented_members(h: Harness) -> None:
    ...
    assert sale["customer_id"] == c.SEED_CUSTOMER_ADA_ID
    ...
    assert sale["line_items"][0]["product"] == {"id": c.SEED_PRODUCT_TRAIL_MIX_ID}
...
def test_the_totals_are_computed_from_the_line_items(h: Harness) -> None:
    """The seeded closed sale: 2 x 10.87 + 1 x 21.65 = 43.39 excluding tax,
    2 x 1.63 + 1 x 3.25 = 6.51 of tax, 49.90 including -- which is the
    catalogue's own 2 x 12.50 + 24.90, because the seeded lines carry the
    products' own prices."""
    totals = h.get(h.path(f"{SALES}/{c.SEED_SALE_CLOSED_ID}")).json()["data"]["totals"]
    assert totals == {"price": 43.39, "price_incl_tax": 49.9, "tax": 6.51, "loyalty": 0.0, "surcharge": 0.0}
...
def test_the_taxes_block_groups_the_lines_by_tax_id(h: Harness) -> None:
    ...
    assert taxes == [{"id": c.SEED_TAX_ID, "tax": 6.51}]
...
def test_a_sale_may_name_a_customer_that_exists(h: Harness) -> None:
    assert h.post(h.path(SALES), body(customer_id=c.SEED_CUSTOMER_ADA_ID)).status == 200
...
def test_a_return_answers_a_new_parked_sale_with_negated_lines(h: Harness) -> None:
    ...
    assert returned["totals"]["price_incl_tax"] == -49.9
...
#
# The shipped scenario carries the inventory records these move: the sibling
# slice of konyklabs/roadmap#94 seeds ten of them, two per stocked product per
# outlet. Before the two slices were merged this file inserted its own record,
# because the collection was empty here; it reads the seeded ones now, which is
# the ordinary path and the one a consumer meets.

SEEDED_MAIN_TRAIL_MIX_LEVEL = 24.0
SEEDED_MAIN_SOCKS_LEVEL = 40.0


def record_id(h: Harness, product_id: str, outlet_id: str) -> str:
    """The seeded inventory record for one product at one outlet -- found the
    way the surface finds it, by the pair rather than by a written-down id."""
    rows = h.unit.context.store.collection(COL.inventory).all()
    matched = [row for row in rows if row["product_id"] == product_id and row["outlet_id"] == outlet_id]
    assert len(matched) == 1, matched
    return str(matched[0]["id"])


def level_of(h: Harness, record: str) -> Any:
    stored = h.unit.context.store.collection(COL.inventory).get(record)
    assert stored is not None
    return float(stored["current_inventory_level"])


def main_trail_mix(h: Harness) -> str:
    return record_id(h, c.SEED_PRODUCT_TRAIL_MIX_ID, c.SEED_OUTLET_MAIN_ID)


def test_closing_a_sale_draws_the_outlets_stock(h: Harness) -> None:
    """The register the sale names decides the outlet, so the main register's
    sale moves the main outlet's record and leaves the second outlet's."""
    record = main_trail_mix(h)
    second = record_id(h, c.SEED_PRODUCT_TRAIL_MIX_ID, c.SEED_OUTLET_SECOND_ID)
    before_second = level_of(h, second)
    assert h.post(h.path(SALES), body(state="closed", line_items=[line(quantity=3)])).status == 200
    assert level_of(h, record) == SEEDED_MAIN_TRAIL_MIX_LEVEL - 3
    assert level_of(h, second) == before_second
...
def test_a_parked_sale_draws_nothing_until_it_closes(h: Harness) -> None:
    ...
    assert level_of(h, record) == SEEDED_MAIN_TRAIL_MIX_LEVEL
...
def test_a_return_gives_the_stock_back(h: Harness) -> None:
    """Its line quantities are negative, so the same subtraction adds. The
    seeded closed sale sold two of one product and one of another, so closing
    its return puts both back -- the seeded levels never moved for the seeded
    sale itself, which is a scenario fact and not an accounting one: the levels
    are stated outright and the sales stand beside them as history."""
    trail_mix = main_trail_mix(h)
    socks = record_id(h, c.SEED_PRODUCT_SOCKS_ID, c.SEED_OUTLET_MAIN_ID)
    returned = h.post(h.path(f"{SALES}/{c.SEED_SALE_CLOSED_ID}/actions/return"), "{}").json()["data"]
    refund = body(
        state="closed",
        line_items=[
    ...
    assert level_of(h, trail_mix) == SEEDED_MAIN_TRAIL_MIX_LEVEL + 2
    assert level_of(h, socks) == SEEDED_MAIN_SOCKS_LEVEL + 1


def test_the_stock_move_fires_inventory_update(h: Harness) -> None:
    """One delivery, for the one record that moved -- and the level it carries
    is the level after the draw, because the payload is the committed entity."""
...
def test_a_product_with_no_record_for_the_outlet_is_skipped_silently(h: Harness) -> None:
    """A product that TRACKS inventory (``has_inventory: true``, which is the
    create's default) but has no record at the sale's outlet is skipped, and
    the sale closes anyway.

    JUDGMENT, and it is deliberate: the absence of a record is a fact about the
    SCENARIO -- this outlet has never counted this product -- and not an error
    the caller made. Refusing the sale would stop a till ringing up a line the
    real API rings up; writing a record at zero would put a number in the store
    that nobody counted. So the effect is skipped and nothing is said about it.
    """
    created = h.post(h.path("/products"), json.dumps({"name": "Unstocked Cap", "price_including_tax": 29.5}))
    assert created.status == 200, created.text
    product_id = created.json()["data"][0]
    stored = h.unit.context.store.collection(COL.products).get(product_id)
    assert stored is not None and stored["has_inventory"] is True
    rows = h.unit.context.store.collection(COL.inventory).all()
    assert [row for row in rows if row["product_id"] == product_id] == []

    subscribe(h, "inventory.update")
    answered = h.post(h.path(SALES), body(state="closed", line_items=[line(product_id=product_id, quantity=3)]))
    assert answered.status == 200, answered.text
    assert h.deliveries() == []
