...
def test_conformance(spec: CheckSpec, profile: str, conformance_target: ConformanceTarget) -> None:
    ...
    if result.outcome is Outcome.SKIP:
        assert skip_is_declared(conformance_target, spec.id, profile), (
            f"{result.case_id} skipped for a reason {conformance_target.name} does not declare: {result.detail}"
        )
...
@pytest.mark.conformance
def test_the_aggregate_run_is_strict_clean(conformance_target: ConformanceTarget) -> None:
    ...
    """
    report = run_conformance(conformance_target, transports=("inprocess",), strict=True)
